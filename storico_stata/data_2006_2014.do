...
//for 2006 and 2014 for all cutoff values
...
forval i=1/11{
	...
	preserve
	if `i' == 1{
		drop if year != 2002 & year != 2004 & year!=2006
	}
	if `i' == 2{
		drop if year != 2008 & year != 2010 & year != 2012 & year!=2014
	}
	...
	forval j=1/3{
		...
		//Ratio
		matrix final_table_resid_cons[3+(`j'-1)*3,`i'] = final_table_resid_cons[2+(`j'-1)*3,`i']/final_table_resid_cons[1+(`j'-1)*3,`i']
	}
	restore
} 

//Store tables
esttab matrix(final_table_inc) using income_2002.txt, replace
esttab matrix(final_table_cons) using consumption_2002.txt, replace
esttab matrix(final_table_resid_inc) using resid_income_2002.txt, replace
esttab matrix(final_table_resid_cons) using resid_consumption_2002.txt, replace
