...
/******CALCULATE INCOME AND CONSUMPTION RATIOS**********************************/
...
forval i = 1/`N'{
	...
	if `curr_weight' > cutoffs[1,`curr_cutoff']*.01*total_weight[1]{		
		//For consumption if you take the household at the cutoff you're subject to wild swings. Take instead avg of whole percentile
		local weighted_sum_inc = 0
		local weighted_sum_cons = 0
		local obs_counter = 0
		local pctile_weight = 0
		//Sum values until weight of summed vals is equal to 1% of total weight. Then take weighted avg of consumption
		forval j = `i'/`N'{
			local weighted_sum_inc = `weighted_sum_inc' + income[`j']*weight[`j']
			local weighted_sum_cons = `weighted_sum_cons' + consumption[`j']*weight[`j']
			local obs_counter = `obs_counter' + 1
			local pctile_weight = `pctile_weight' + weight[`j']
			if `pctile_weight' > total_weight[1]*.01{
				matrix income_pctile_vals[`curr_cutoff',1] = `weighted_sum_inc'/`pctile_weight'
				matrix consump_pctile_vals[`curr_cutoff',1] = `weighted_sum_cons'/`pctile_weight'
				continue, break
			}
		}	
...
forval i = 1/`N'{
	...
	if `curr_weight' > cutoffs[1,`curr_cutoff']*.01*total_weight[1]{
		...
		forval j = `i'/`N'{
			...
			local pctile_weight = `pctile_weight' + weight[`j']
			if `pctile_weight' > total_weight[1]*.01{
				matrix income_pctile_vals[`curr_cutoff',2] = `weighted_sum_inc'/`pctile_weight'
				matrix consump_pctile_vals[`curr_cutoff',2] = `weighted_sum_cons'/`pctile_weight'
				continue, break
			}
		}
...
forval i = 1/`N'{
	...
	if `curr_weight' > cutoffs[1,`curr_cutoff']*.01*total_weight[1]{
		...
		//Sum values until weight of summed vals is equal to 1% of total weight. Then take weighted avg of consumption
		forval j = `i'/`N'{
			local weighted_sum_inc = `weighted_sum_inc' + resid_income[`j']*weight[`j']
			local weighted_sum_cons = `weighted_sum_cons' + resid_consump[`j']*weight[`j']
			local obs_counter = `obs_counter' + 1
			local pctile_weight = `pctile_weight' + weight[`j']
			if `pctile_weight' > total_weight[1]*.01{
				matrix resid_income_pctile_vals[`curr_cutoff',1] = `weighted_sum_inc'/`pctile_weight'
				matrix resid_consump_pctile_vals[`curr_cutoff',1] = `weighted_sum_cons'/`pctile_weight'
				continue, break
			}
		}
...
forval i = 1/`N'{
	...
	if `curr_weight' > cutoffs[1,`curr_cutoff']*.01*total_weight[1]{
		...
		forval j = `i'/`N'{
			...
			local pctile_weight = `pctile_weight' + weight[`j']
			if `pctile_weight' > total_weight[1]*.01{
				matrix resid_income_pctile_vals[`curr_cutoff',2] = `weighted_sum_inc'/`pctile_weight'
				matrix resid_consump_pctile_vals[`curr_cutoff',2] = `weighted_sum_cons'/`pctile_weight'
				continue, break
			}
		}
...
matrix final_table[3,1] = consump_pctile_vals[3,2]
matrix final_table[3,2] = consump_pctile_vals[3,2]

/*
Eventually put above 40ish lines of code into a loop
forval i = 1/3{
	forval j = 1/8{
		if `i' < 2 & `j' == 1
		matrix final_table[`i'][`j'] = income_ratios[`i',1]
